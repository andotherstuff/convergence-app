...
export function NoteContent({
  ...
  const tokens = useMemo(() => {
    ...
    // Collapse excessive whitespace around block-level tokens (link-preview, nevent embed, etc.)
  ...
  return (
    <div dir="auto" className={cn('whitespace-pre-wrap wrap-break-word overflow-hidden', className, isEmojiOnly && 'text-5xl leading-tight')}>
        ...
        switch (token.type) {
          ...
          case 'image-embed': {
            if (disableEmbeds || disableMediaEmbeds) {
              // In preview contexts, replace image URLs with a newline so
              // text flow is preserved without showing raw URLs.
              return <span key={i}>{'\n'}</span>;
            }
